    ...
    case 'addreply': // ------------------------ ADD REPLY ---------------------------------
        ...
        if ($text != '') {
            ...
            $sql = 'INSERT INTO cboards VALUES (\''.mysql_escape_string($cix).'\', \'0\', \''.mysql_escape_string(
                    $thread
                ).'\', \''.$usrid.'\', \''.mysql_escape_string($usr['name']).'\', \''.mysql_escape_string(
                    $usr['clusterstat']
                ).'\', \''.mysql_escape_string($ts).'\', \''.mysql_escape_string($title).'\', \''.mysql_escape_string(
                    $text
                ).'\', \'-1\')';
            db_query($sql);
...
<tr valign="top"><td>'.$data['content'].'</td></tr>
        ...
        showform('addreply');
        echo '</div>';
        echo '<p><a href="cboard.php?sid='.$sid.'&m=board">Zur�ck</a></p>'."\n";
