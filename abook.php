...
<form name="formular">';
        ...
        function list_items($g)
        {
            ...
            $r = db_query(
                'SELECT u.id AS u_id, u.name AS u_name, a.id AS entry FROM abook_entrys a JOIN users u ON a.remote_user=u.id WHERE a.user='.$usrid.' AND a.group='.$g
            );
            while ($entry = mysql_fetch_object($r)) {
                echo '<tr><td><a href="user.php?a=info&sid='.$sid.'&user='.$entry->uid.'">'.$entry->u_name.'</a></td>';
                echo '<td><input type="button" value="W&auml;hlen" onclick="choose(\''.str_replace(
                        '\'',
                        '\\\'',
                        $entry->u_name
                    ).'\')"></td></tr>';
...
<tr><td colspan="2" align="right"><input type="submit" value="Hinzuf&uuml;gen" /></td></tr>
        ...
        function admin_list($ix)
        {
            ...
            eval('$ch'.$ix.'=\' selected\';');
            $r = db_query(
                'SELECT u.id, u.name, a.id AS entry FROM abook_entrys a JOIN users u ON a.remote_user=u.id WHERE a.user='.$usrid.' AND a.group='.$ix
            );
    ...
    case 'saveadmin': //------------------------- SAVE ADMIN -------------------------------
        ...
        while ($entry = mysql_fetch_object($r)) {
            if ($_POST['u'.$entry->id] == 'yes') {
                ...
                db_query(
                    'UPDATE `abook_entrys` SET `group`='.(int)$_REQUEST['group'.$entry->id].' WHERE `user`='.$usrid.' AND `id`='.$entry->id.' LIMIT 1'
                );
            }
        }
