...
';
    ...
    case 'create_final': // -------------------------------- CREATE FINAL -------------------------------
        ...
        @unlink($fn);
        db_query(
            'INSERT INTO distr_attacks VALUES(\'0\', \''.mysql_escape_string(
                $clusterid
            ).'\', \''.$pcid.'\', \''.$usrid.'\', \''.mysql_escape_string($target).'\', \''.mysql_escape_string(
                $item
            ).'\');'
        );
        $id = mysql_insert_id();
        db_query(
            'INSERT INTO da_participants SET relative='.mysql_escape_string($id).', pc='.mysql_escape_string(
                $pc['id']
            ).', ip=\''.mysql_escape_string($pc['ip']).'\', owner='.$usrid.', owner_name=\''.mysql_escape_string(
                $usr['name']
            ).'\';'
        );
#echo mysql_error();
    ...
    case 'join': // -------------------------------- JOIN -------------------------------
        ...
        if (isset($a['id']) == true && $a['cluster'] == $clusterid) {
            ...
            while ($x = mysql_fetch_assoc($sql)) {
                ...
                $result = db_query(
                    'SELECT relative FROM da_participants WHERE relative=\''.$a['id'].'\' AND pc=\''.$x['id'].'\';'
                );
            ...
            while ($xpc = mysql_fetch_assoc($sql)) {
                ...
                db_query(
                    'INSERT INTO da_participants SET relative=\''.mysql_escape_string(
                        ...
                        $xpc['ip']
                    ).'\', owner=\''.$usrid.'\', owner_name=\''.mysql_escape_string($usr['name']).'\';'
                );
                $cnt++;
                #if($gcnt > 100) break;
            }
...
############
                ...
                if ($dif < 10 || $remote[$a['item']] == $lv || ($a['item'] == 'cpu' && $remote[$a['item']] == array_search(
                    ...
                    addsysmsg(
                        $owner['id'],
                        'Von '.$total.' PCs von Spielern des Clusters [cluster='.$clusterid.']'.$cluster['code'].'[/cluster] wurde eine Distributed Attack gegen dich ausgef&uuml;hrt!<br />Sie konnte aber abgewehrt werden: Auf deinem PC 10.47.'.$remote['ip'].' ('.$remote['name'].') gab es keinen Schaden!'
                    );
                } else {
                    ...
                    addsysmsg(
                        $remote['owner'],
                        'Von '.$total.' PCs von Spielern des Clusters [cluster='.$clusterid.']'.$cluster['code'].'[/cluster] wurde eine Distributed Attack gegen dich ausgef&uuml;hrt!<br />Sie war erfolgreich: Auf deinem PC 10.47.'.$remote['ip'].' ('.$remote['name'].') wurde '.idtoname(
                            $a['item']
