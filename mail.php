...
function newmailcount($save = true)
{
    ...
    if ($save) {
        db_query('UPDATE users SET newmail=\''.$anz.','.$anz2.'\' WHERE id=\''.$usrid.'\';');
    }
...
{ // ------------------------- MAIL LIST -----------------------------
    ...
    $result = db_query(
        'SELECT * FROM mails WHERE user=\''.$usrid.'\' AND box=\''.mysql_escape_string($boxid).'\' ORDER BY time DESC'
    );
    $anz = mysql_num_rows($result);
...
';
        ...
        if ($sysmsgs != '') { ######### Tabllenbeschreiftung falsch ############
...
';
                ...
                if ($text == '') {
                    ...
                    if ($usr['lastmail'] + 15 <= $ts || $localhost || $usrid == 1) {
                        ...
                        if ($cnt < getmaxmailsforuser('in', $recip['bigacc'])) {
                            ...
                            db_query(
                                'INSERT INTO mails VALUES (\'0\', \''.$usrid.'\', \''.mysql_escape_string(
                                    $recip['id']
                                ).'\', \''.mysql_escape_string($recip['name']).'\', \''.mysql_escape_string(
                                    $ts
                            ...
                            $r = db_query(
                                'SELECT mail FROM mails WHERE user='.$usrid.' AND box=\'out\' ORDER BY time ASC'
                            );
                            $cnt = mysql_num_rows($r);
                            $max = getmaxmails('out');
                            if ($cnt > $max) {
    ...
    case 'read': //------------------------- READ -------------------------------
        ...
        $result = db_query(
            'SELECT * FROM mails WHERE USER=\''.$usrid.'\' AND mail LIKE \''.mysql_escape_string($msg).'\' LIMIT 1;'
        );
...
';
    ...
    case 'reply': //------------------------- REPLY -------------------------------
        ...
        newmailform(
            $sender['name'],
            $subject,
            "\n".LF."\n".LF.'------------------------'.LF.$sender['name'].' schrieb:'.LF.$text,
            false
        );
        echo '</div>'."\n";
        createlayout_bottom();
...
';
        function transmit2_mail_list($box)
        {
            ...
            $result = db_query(
                'SELECT * FROM mails WHERE user=\''.$usrid.'\' AND box=\''.mysql_escape_string(
                    $box
                ).'\' ORDER BY time DESC'
            );
            $anz = mysql_num_rows($result);
