        ...
        switch ($_REQUEST[type]) {
            case 'pc':
                ...
                if ($sql != '') {
                    ...
                    db_query(
                        'INSERT INTO logs SET type=\'adminedit\', usr_id='.$usr['id'].', payload=\'PC '.$pc['id'].': '.mysql_escape_string(
                            $sql
                        ).'\';'
                    );
                }
            ...
            case 'user':
                ...
                if ($sql != '') {
                    ...
                    db_query(
                        'INSERT INTO logs SET type=\'adminedit\', usr_id='.$usr['id'].', payload=\'USER '.$user['id'].': '.mysql_escape_string(
                            $sql
                        ).'\';'
                    );
                }
            ...
            case 'cluster':
                ...
                if ($sql != '') {
                    ...
                    db_query(
                        'INSERT INTO logs SET type=\'adminedit\', usr_id='.$usr['id'].', payload=\'CLUSTER '.$cluster['id'].': '.mysql_escape_string(
                            $sql
                        ).'\';'
                    );
                }
...
<table><tr><th>Name</th><th>Wie oft</th></tr>';
        foreach ($result as $k => $v) {
            ...
            if (mysql_num_rows($r) == 1) {
                $name = '<a href="user.php?page=info&sid='.$sid.'&user='.$k.'">'.@mysql_result($r, 0, 'name').'</a>';
            } else {
                $name = $k;
            }
...
<input type=submit value=" ACCOUNT L&Ouml;SCHEN ">
    ...
    case 'lockacc':  // ----------------------------------- LOCK ACC --------------------------------
        ...
        $uid = (int)$_REQUEST['user'];
        db_query(
            'INSERT INTO logs SET type=\'lockuser\', usr_id=\''.$uid.'\', payload=\'locked by '.mysql_escape_string(
                $usr['name']
            ).'\';'
        );
        mysql_query(
            'UPDATE users SET locked=\'yes\',locked_till=0,locked_by=\''.$usr['name'].'\',locked_reason=\'kein Grund angegeben\' WHERE id=\''.$uid.'\' LIMIT 1;'
        );
  ...
  <tr><th>Sperren bis:</th><td><input type="text" name="till" size="30" value="'.strftime(
                '%x %X',
                time() + 24 * 60 * 60
            ).'"><br />0 eingeben f�r unbegrenzt</td></tr>
  ...
  <tr><th colspan="2" align="right"><input type="submit" value=" Sperren! "></tr>
    ...
    case 'lockaccex2':  // ----------------------------------- LOCK ACC EX 2 --------------------------------
        ...
        $uid = (int)$_REQUEST['user'];
        db_query(
            'INSERT INTO logs SET type=\'lockuser\', usr_id=\''.$uid.'\', payload=\'locked ex by '.mysql_escape_string(
                $usr['name']
            ).'\';'
        );
        if ($_POST['till'] == '0') {
        ...
        mysql_query(
            'UPDATE users SET locked=\'yes\',locked_till='.$ts.',locked_by=\''.$usr['name'].'\',locked_reason=\''.mysql_escape_string(
                $_POST['reason']
...
{  // --------------------- ID TO TEXT ------------------
    $items = array(
        ...
        'sid_ip' => 'IP-Adresse zur SID',
        'locked_till' => 'gesperrt bis',
        'locked_by' => 'gesperrt durch',
        'locked_reason' => 'Grund der Sperrung',
    );
    $s = idtoname($id);
    if ($s == '') {
        $s = $items[$id];
