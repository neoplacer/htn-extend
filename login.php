...
if ($action == 'login') {
    ...
    if ((int)@file_get('data/calc-time.dat') <= time() || @file_get(
            'data/calc-running.dat'
        ) == 'yes' && $_POST['bypass'] != 1
    ) {
...
<input type="submit" value="Ignorieren" />
    ...
    if (@mysql_num_rows($r) == 1) {
        ...
        if ($usr['password'] != $pwd) {
            simple_message('Passwort f�r diesen Account falsch!');
            db_query(
                'INSERT INTO logs SET type=\'badlogin\', usr_id=\''.$usr['id'].'\', payload=\'pwd='.mysql_escape_string(
                    $_REQUEST['pwd']
                ).', ip='.mysql_escape_string($_SERVER["REMOTE_ADDR"]).'\';'
            ); #ip= added by zakx at Wed Oct 27 16:42:43 GMT-2 2004
            exit;
        }
        ...
        if (count($_GET) < 2 || $usr['bigacc'] == 'yes') {
            ...
            if ($usr['locked'] == 'yes' && ($usr['locked_till'] > time() || $usr['locked_till'] == 0)) {
                if ($usr['locked_till'] == 0) {
                    ...
                    $time = ' bis '.nicetime($usr['locked_till']);
                }
...
<br /><br />Der Grund daf�r ist: <em>'.$usr['locked_reason'].'</em><br />Bei Fragen, Beschwerden und wenn
du die genauen Gr&uuml;nde wissen willst, schick eine Email an <a href="mailto:'.$admin_email.'?subject='.urlencode(
                'Sperrung des Accounts '.$usr['name']
            ).'">'.$admin_email.'</a>!
</body></html>';
                exit;
            }
            ...
            @db_query(
                'INSERT INTO logins ( ip, usr_id, time, user_agent ) VALUES ( \''.mysql_escape_string(
                    $ip2
                ).'\', \''.$usrid.'\', \''.mysql_escape_string(time()).'\', \''.mysql_escape_string(
                    $_SERVER['HTTP_USER_AGENT']
                ).'\' );'
            );
            db_query(
                'UPDATE users SET sid=\''.mysql_escape_string($sid).'\', login_time=\''.time(
                ).'\', sid_ip=\''.mysql_escape_string(
                    $ip
                ).'\', noipcheck=\''.$noipcheck.'\', last_verified=0, sid_pc='.$pcid.', sid_lastcall='.time(
