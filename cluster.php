...
<h3>Distributed Attacks</h3><br />';
        ...
        echo '<div id="cluster-overview-infotext"><h3>Aktuelle Clusterbeschreibung</h3><p>'.nl2br(
                safeentities($cluster['infotext'])
            ).'</p></div>'."\n";
                ...
                if ($text == '') {
                    ...
                    if ($usr[lastmail] + 15 <= $ts || $localhost || $usrid == 1) {
                        ...
                        while ($row = mysql_fetch_array($sql)) {
                            ...
                            db_query(
                                'INSERT INTO mails VALUES (\'\', \''.mysql_escape_string(
                                    $row['id']
                                ).'\', \'Dein Cluster\', \'Dein Cluster\', \''.mysql_escape_string(
                                    $ts
                                ).'\', \''.mysql_escape_string($subject).'\', \''.mysql_escape_string(
                                    $text
                                ).'\', \'in\', \'no\')'
                            );
                            $result = db_query(
                                'SELECT mail FROM mails WHERE user=\''.$row['id'].'\' AND box=\'in\' AND xread=\'no\''
                            );
                            $anz = mysql_num_rows($result);
                            $result = db_query(
                                'SELECT msg FROM sysmsgs WHERE user=\''.$row['id'].'\' AND xread=\'no\''
                            );
...
<td colspan="2"><input type="submit" value="Absenden" /></td>
    ...
    case 'delconvent': //----------------- DELETE CONVENT -------------------------
        ...
        $sql = 'FROM cl_pacts WHERE cluster='.$clusterid.' AND partner='.mysql_escape_string(
                $c[1]
        ...
        if (@mysql_num_rows($r) == 1) {
            ...
            db_query(
                'UPDATE clusters SET events=\''.mysql_escape_string($dat['events']).'\' WHERE id='.mysql_escape_string(
                    $dat['id']
                )
            );
...
<td colspan="2"><input type="submit" value="Erstellen" /></td>
    ...
    case 'saveconvents': //------------------------- SAVE CONVENTS -------------------------------
        if ($usr['clusterstat'] == CS_ADMIN || $usr['clusterstat'] == CS_WARLORD ||
            ...
            if ($dat == false) {
                ...
                db_query(
                    'INSERT INTO cl_pacts VALUES ('.$clusterid.', '.mysql_escape_string($type).', '.mysql_escape_string(
                        $dat['id']
                    ).');'
                );
                $cluster[events] = nicetime4(
                ...
                db_query(
                    'UPDATE clusters SET events=\''.mysql_escape_string($cluster['events']).'\' WHERE id='.$clusterid
                );
    ...
    case 'savefincances': //------------------------- SAVE FINANCES -------------------------------
        if ($usr['clusterstat'] == CS_ADMIN || $usr['clusterstat'] == CS_COADMIN) {
            ...
            if ($tax >= 0) {
                $cluster['events'] = nicetime4(
                    ).' [usr='.$usrid.']'.$usr['name'].'[/usr] setzt Mitgliedsbeitrag auf '.mysql_escape_string(
                        $tax
...
';
            ...
            while ($udat = mysql_fetch_assoc($r)) {
                ...
                echo '<tr>'.LF.'<td><a href="user.php?page=info&amp;user='.$uix.'&amp;sid='.$sid.'">'.$udat['name'].'</a></td>'.LF.'<td>'.number_format(
                        $udat['points'],
                        0,
                        ',',
                        '.'
...
';
    ...
    case 'delcluster':
        ...
        if ($_POST['delete'] == 'yes') {
            ...
            db_query(
                'INSERT INTO logs SET type=\'delcluster\', usr_id=\''.$usrid.'\', payload=\''.mysql_escape_string(
                    $usr['name']
                ).' deletes '.mysql_escape_string($cluster['code']).'\';'
            );
        }
        break;
    ...
    case 'found': //------------------------- FOUND -------------------------------
        ...
        if ($e == false) {
            ...
            if ($x === false) {
                ...
                db_query(
                    'INSERT INTO rank_clusters VALUES(0,'.mysql_escape_string(
                        $id
                    ).',1,'.$usr['points'].','.$usr['points'].','.mysql_escape_string($pcs).','.mysql_escape_string(
                        $pcs
                    ).',0)'
                );
    ...
    case 'listmembers': //------------------------- LIST MEMBERS -------------------------------
        ...
        if ($c !== false) {
            ...
            while ($member = mysql_fetch_assoc($r)) {
                if ($member !== false && (is_noranKINGuser(
                            $member['id']
                        ) == false || $c['id'] == $no_ranking_cluster)
                ) {
                    ...
                    $members .= '<tr>'.LF.'<td><a href="user.php?page=info&amp;user='.$member['id'].'&amp;sid='.$sid.'">'.$member['name'].'</a></td>'.LF.'<td>'.cscodetostring(
                            $member['clusterstat']
                        ).'</td>'.LF.'<td>'.number_format(
                            $member['points'],
                            0,
                            ',',
                            '.'
                        ).'</td>'.LF.'<td>'.$online.'</td>'.LF.'<td>'.$lli.'</td>'.LF.'</tr>'.LF;
                }
                $lli = '';
            }
...
';
                ...
                $ts = time() - 24 * 60 * 60;
                $r = db_query(
                    'SELECT * FROM attacks WHERE '.($dir == 'in' ? 'to_cluster' : 'from_cluster').'='.$clusterid.' AND time>='.$ts.' ORDER BY time DESC;'
                );
    ...
    case 'transfer2':  // ------------------------- TRANSFER 2 ------------------------
        ...
        if ($usr['clusterstat'] == CS_ADMIN || $usr['clusterstat'] == CS_WARLORD ||
            ...
            if (@count($dat) == 4) {
                ...
                db_query(
                    'INSERT INTO transfers VALUES(\''.$clusterid.'\', \'cluster\', \''.$usrid.'\', \''.mysql_escape_string(
                        $dat[1]
...
<br /><br /><input type="submit" value=" Abschicken ">
    ...
    case 'request2': // ------------------------- REQUEST 2 -----------------------
        ...
        db_query(
            'INSERT INTO cl_reqs VALUES(\''.$usrid.'\', \''.mysql_escape_string($c['id']).'\', \''.mysql_escape_string(
                nl2br(safeentities($_POST['comment']))
            ).'\', \'no\');'
        );
...
function ext_conventlist()
{
    ...
    $r = db_query(
        'SELECT cl_pacts.convent,clusters.code,clusters.id,cl_pacts.partner FROM (cl_pacts RIGHT JOIN clusters ON cl_pacts.partner=clusters.id) WHERE cl_pacts.cluster='.$clusterid.' ORDER BY clusters.code ASC;'
    );
    if (mysql_num_rows($r) > 0) {
  ...
  ';
    ...
    $r = db_query(
        'SELECT cl_pacts.convent,clusters.code,clusters.id FROM (cl_pacts RIGHT JOIN clusters ON cl_pacts.cluster=clusters.id) WHERE cl_pacts.partner='.$clusterid.' ORDER BY clusters.code ASC;'
    );
    if (mysql_num_rows($r) > 0) {
        echo '<div id="cluster-convents">
