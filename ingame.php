...
if (preg_match("/^[a-z0-9]+$/i", $sid) == false || $sid == '') {
    die('FATAL ERROR: INVALID SESSION-ID!');
}
...
function badsession($s)
{
    ...
    $sid = '';
    simple_message(
        'Sitzung ung&uuml;ltig!<br />Bitte auf der <a href="./">Startseite</a> neu einloggen!<br /><br /><span style="font-size:9pt;">Grund: '.$s.'</span>'
    );
    exit;
}
...
if ($usr['sid_ip'] != $ip && $usr['sid_ip'] != 'noip') {
    /* falsche IP-Adresse */
    badsession(
        'Deine IP ist nicht dieser Session-ID zugeordnet!<br />Aktiviere die Option "IP-�berpr�fung deaktivieren" beim Login.'
    );
}

if ($usr['locked'] == 'yes' && ($usr['locked_till'] > time() || $usr['locked_till'] == 0)) {
    badsession('Neu einloggen, du b�ser Vogel!');
}

if ($usr['login_time'] < time() - 5 * 3600 || $usr['sid_lastcall'] < time() - 3600) {
    db_query('UPDATE `users` SET `sid`=\'\' WHERE `id`='.$usr['id'].' LIMIT 1');
    badsession('Maximale Session-Dauer oder Inaktivit�ts-Zeit �berschritten!');
}
...
if ($usr['last_verified'] + 3600 < time() && $enable_visual_confirmation) {
    ...
    createlayout_top('Session best�tigen!');
    echo '<div class="content"><h2>Session best�tigen</h2>
  <div id="confirm-session">
  <h3>Bitte die Session best�tigen</h3>
  <p><strong>Um das Spiel vor Bots zu sch�tzen, muss jeder Spieler einmal pro Stunde seine Anwesenheit/Menschlichkeit
  best�tigen!</strong><br />Tippen Sie die Zahlen/Buchstaben von links einfach rechts nochmal ein und klicken Sie dann auf OK.</p>
...
if ($usr['liu'] > $usr['lic']) {
    $unread = (int)@mysql_num_rows(
        db_query('SELECT mail FROM mails WHERE user=\''.$usrid.'\' AND box=\'in\' AND xread=\'no\';')
    );
...
{ //------------------------- Save PC -------------------------------
    ...
    $s = trim($s, ',');
    if (strlen($s) > 4) {
        db_query('UPDATE pcs SET '.$s.' WHERE id=\''.$pcid.'\'');
    }
}
