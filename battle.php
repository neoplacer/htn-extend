...
<input type="reset" value="  Reset  " /> <input type=submit value=" Weiter " /></th></tr>
    ...
    case 'opc_submit': // --------------- OPC Submit -----------------------
        ...
        $showvertraege = mysql_query(
            "SELECT * FROM cl_pacts WHERE cluster=".$usr[cluster]." && partner=".$target_owner['cluster']." && convent!=1"
        );
        ...
        if ($kill_friends['battle'] == "1") {
            if (@mysql_num_rows($showvertraege) > 0) {
                $e .= 'Du kannst keine Vertragspartner angreifen!';
            }
            if ($usr[cluster] == $target_owner[cluster]) {
                $e .= 'Du kannst keine Clusterkollegen angreifen!';
            }
        }
        ...
        if ($GAME_MODE == '2.1') {
            if ($hijack && count(explode(',', $usr['pcs'])) >= MAX_PCS_PER_USER) {
                $e .= 'Du hast bereits die maximale Anzahl von PCs erreicht!<br />';
            }
        }
        ...
        if ($hijack) {
            ...
            if (count($userpcs) >= 10) {
                ...
                $usrname = $usr['name'];
                $abfragex = @mysql_num_rows(
                    db_query('SELECT * FROM pcs WHERE owner_name = "'.$usrname.'" AND points = "1024"')
                );
                if ($abfragex < 5) {
...
# Durch Trojaner deaktivierte Items beachten!!
        ...
        if ($owner['cluster'] == $no_ranking_cluster) {
                ...
                if ($enemy_points <= ($my_points * (10 / 100))) {
                    $r = db_query(
                        'SELECT * FROM `attacks` WHERE `from_usr`=\''.mysql_escape_string(
                            $owner['id']
                        ).'\' AND `to_usr`=\''.$usrid.'\' AND `type`<>\'scan\';'
                    );
        ...
        if (($e == '' && $scan == 0) & ($owner['points'] < MIN_ATTACK_POINTS && $owner['login_time'] + MIN_INACTIVE_TIME > time(
                ))
        ) {
            $e .= 'Du kannst keinen User mit weniger als '.$MIN_ATTACK_POINTS.' Punkten angreifen, der sich in den letzten 3 Tagen eingeloggt hat!<br />';
        }
...
<ul><li><b>Der Angriff richtet sich gegen 10.47.'.$target['ip'].' ('.$target['name'].'). Dieser Rechner geh&ouml;rt ';
            ...
            file_put(
                ...
                implode('|', array($target[id], $scan, $trojan, $smash, $block, $hijack, $opt, $msg, $cost, $pc['id']))
            );
...
<p><form><input type=button onclick="abortattack()" value=" Abbrechen "></form></p>
    ...
    case 'execute': // --------------------------------- EXECUTE ---------------------------------
        ...
        if ($remote['di'] != '' && $remote['dt'] > time() && $trojan == 1 && substr($opt, 0, 10) == 'DEACTIVATE') {
            simple_message(
                'Auf dem Zielrechner ist bereits ein Item deaktiviert! Es k&ouml;nnen nicht gleichzeitig mehrere Items auf einem PC deaktiviert sein!<br />'
            );
            exit;
        }
...
<br /><input type=submit value="Speichern">
            ...
            $attack = getAttack(
                '$local[rh] * 2 + $local[sdk] * 1.5 + $local[mk] * 1.5 + $local[ips] + $local[cpu] / 1.8 + $local[ram] / 1.8'
            );
        ...
        function addtoattacklist()
        {
            ...
            $ts = time();
            $rem_own = getuser($remote2['owner']);
            $sql = 'INSERT INTO attacks VALUES(0, '.$pcid.', '.$usrid.', '.(int)$usr['cluster'].', '.(int)$remote2['id'].', '.(int)$rem_own['id'].', '.(int)$rem_own['cluster'].', \''.mysql_escape_string(
                    $t
                ).'\', \''.mysql_escape_string($opt).'\', \''.mysql_escape_string(
                    $success
                ).'\', \''.mysql_escape_string($noticed).'\', \''.mysql_escape_string($ts).'\', \''.$payload.'\');';
            db_query($sql);
...
#    echo 'acnt='.$acnt.' scnt='.$scnt.' ncnt='.$ncnt.'<br>';
                ...
                db_query(
                    'UPDATE clusters SET srate_total_cnt=srate_total_cnt+1, srate_success_cnt='.$scnt.', srate_noticed_cnt='.$ncnt.' WHERE id='.$cluster['id'].';'
                );
            }
